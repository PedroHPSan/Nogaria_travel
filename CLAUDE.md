...
### Supabase: database exists, frontend is still not wired to it
...
**Still true today:** `src/` contains zero Supabase imports, zero `import.meta.env` reads, and zero network calls — the running app is 100% localStorage per `TripContext.tsx`. Wiring the frontend to this schema (swapping the `useState`/`localStorage` pairs in `TripContext.tsx` for `supabase-js` calls) has not been started.
