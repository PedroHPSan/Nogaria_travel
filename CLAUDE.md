...
External setup (manual, not automatable from the repo): Meta app + WhatsApp Business number + permanent token; `supabase secrets set` for `GEMINI_API_KEY`, `META_WA_TOKEN`, `META_WA_VERIFY_TOKEN`, `META_WA_APP_SECRET`, `CRON_SECRET` (all documented in `env.example`); Vault secrets `project_url`/`cron_secret` then re-run the **two** cron migrations (`daily_digest_cron`, `activity_reminders_cron`); webhook URL pointed at `/functions/v1/whatsapp-webhook`; insert a `whatsapp_configs` row per tenant. The daily digest's first outbound message of the day may require an approved **message template** in Meta Business (utility category).
