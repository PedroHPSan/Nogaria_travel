...
**Still not wired:** within `TripContext.tsx`, `trips`, `participants`, and (as of this migration) `itinerary` are wired to Supabase — see `src/data/useTripsData.ts` and `src/data/useItineraryData.ts`. The rest (`flights`, `accommodations`, `transports`, `gift_cards`, `purchase_items`, `luggages`, `expenses`, `tasks`, `decisions`, `documents`, `loyalty_accounts`, `ai_provider_configs`, `ai_usage_logs`) remain localStorage-only, deliberately out of scope for this pass — swapping their `useState`/`localStorage` pairs for `supabase-js` calls against the tables above hasn't been started. Logging out clears both the Supabase session and every `localStorage` key under `TripContext.tsx`'s `STORAGE_KEY` prefix, but the still-unwired entities don't talk to Supabase yet.
...
- **`whatsapp_configs.timezone` é o relógio de todo o bot, e o default está errado para esta viagem.** O default da coluna é `'America/Sao_Paulo'` (UTC−3), mas os `time_start` do roteiro são hora de Orlando (`America/New_York`, UTC−4 no horário de verão). Com o default, `localMinutesOfDay` devolve uma hora a mais que a realidade e **todo aviso sai 1h adiantado** — e, pior, os avisos cuja janela cai nessa hora de diferença somem: o agendador exige `0 < minutosAté <= lead` e nunca envia retroativo, então um item às 09:00 com lead 30 já está "no passado" para um bot que acha que são 09:04. Aconteceu em produção em 09/09/2026 (corrigido com `update public.whatsapp_configs set timezone = 'America/New_York'`). O mesmo valor governa `digest_time` e a janela de silêncio. A consulta 7 de `supabase/seeds/verificar_epcot_2026-09-09.sql` checa isso.
...
- **`auditEngine.ts` and `AiCopilotView.tsx` are hardcoded to one specific trip.** The audit matches participants by `nickname === 'Pedro'` / `'Gabi'` and `age === 12`, and string-matches the literal timestamp `'2026-09-19T17:30'`. The AI Copilot is not an LLM at all — it's a `setTimeout` plus keyword `if/else` chain returning canned Portuguese text about this trip. The `AiProviderConfig` / `AiUsageLog` settings UI is decorative; no provider is ever called. Generalizing either means removing hardcoded names, ages, or dates. (`MonthCalendar.tsx` no longer hardcodes the month — it derives the grid from `activeTrip.start_date`, but still renders a single month, so trips spanning two months only show the first.)
