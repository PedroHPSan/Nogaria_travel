...
**Avisos por horário.** `supabase/functions/activity-reminders/index.ts` (público, `verify_jwt = false`, gated por `x-cron-secret`), agendado por pg_cron a cada 10 min nos minutos `3,13,...` para não concorrer com o digest (minuto 7). Toda a lógica de janela vive em `_shared/reminderScheduler.ts`, que é **puro e testado** (26 testes) — a function só faz I/O.
- **Janela aberta, não fatiada:** envia quando `0 < minutosAté <= lead`, não quando o item cai numa fatia de 10 min. Execução perdida é recuperada pela seguinte; atividade já iniciada nunca gera aviso atrasado.
...
- **Antecedência em cascata:** `itinerary_items.reminder_minutes_before` (override, `0` desliga) → senão `max(whatsapp_configs.reminder_lead_minutes, recommended_arrival_min_before + 15)`. A granularidade do cron é a *resolução* do aviso, não sua precisão: um aviso de 60 min sai entre 60 e 50 min antes.
