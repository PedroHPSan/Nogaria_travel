...
Layout:
- `supabase/functions/whatsapp-webhook/index.ts` — public (`verify_jwt = false` in `config.toml`). GET handles Meta's `hub.verify_token` challenge; POST verifies `X-Hub-Signature-256` (HMAC-SHA256 with `META_WA_APP_SECRET`, skipped when the secret is unset), extracts text messages, dedupes on `whatsapp_messages.wa_message_id` (unique — Meta retries webhooks), resolves tenant via `whatsapp_configs.phone_number_id`, then calls Gemini with **function calling** and replies.
