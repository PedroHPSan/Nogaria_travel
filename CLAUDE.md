...
| `CRON_SECRET` | digest por WhatsApp | o mesmo valor de `supabase secrets set CRON_SECRET` |
...
**GitHub Actions em billing hold (desde 14/09/2026): os dois workflows acima não rodam.** Nem o push-trigger de `deploy.yml`/`supabase.yml` nem o `workflow_dispatch` manual funcionam enquanto a conta estiver bloqueada — use o fallback manual logo abaixo (`vercel deploy --prod --yes`, `supabase db push --linked`, `supabase functions deploy <name>`) até o billing ser resolvido. O cron recorrente do bot **não** depende disso — ver a próxima seção.

### Cron do bot: Vercel Cron, não mais pg_cron (desde 2026-09-14)

`api/cron/{daily-digest,activity-reminders,activity-checkins}.ts` — três Vercel Functions minúsculas (Node, sem framework, funcionam num projeto Vite como este via a pasta `api/` na raiz) que só repassam um POST para a edge function correspondente do Supabase, com o mesmo header `x-cron-secret` que o pg_net do pg_cron mandava. `vercel.json` agenda os três nos mesmos horários que os antigos jobs de pg_cron usavam (`7 * * * *`, `2,9,16,23,30,37,44,51,58 * * * *`, `5,20,35,50 * * * *`) — ver Vercel → Project → Cron Jobs para o histórico de execuções, em vez de `select * from cron.job_run_details` no Postgres.

Duas env vars novas no Vercel (Project Settings → Environment Variables), nomes diferentes de propósito — são dois sistemas checando cada um o seu próprio segredo:
- `CRON_SECRET` — o Vercel manda `Authorization: Bearer $CRON_SECRET` sozinho quando esta env existe (é assim que `api/cron/_shared.ts` recusa qualquer chamada que não veio do próprio Vercel Cron).
- `SUPABASE_CRON_SECRET` — mesmo valor do `supabase secrets set CRON_SECRET`; é o que vai no header `x-cron-secret` da chamada à edge function.

`20260914190000_move_cron_to_vercel.sql` desagenda os jobs antigos de pg_cron (`whatsapp-daily-digest`, `whatsapp-activity-reminders`, `whatsapp-activity-checkins`) — **rodar os dois lados ao mesmo tempo duplica toda mensagem da família.** `exchange-rate-sync` continua no pg_cron (não é do bot). Motivo da troca: com o GitHub em billing hold, o job manual `whatsapp` de `supabase.yml` (único jeito de forçar um envio fora de hora) ficou inacessível, e a mesma migration desagendou de vez `whatsapp-trip-report-2026-09-14-a-16` — um cron de disparo único agendado com **data fixa já passada** (`0 23 13 9 *`, dia 13/09, quando o pedido só chegou na noite do 14/09): esse padrão (pg_cron com `cron.unschedule` de si mesmo depois de disparar) é frágil por natureza para envio avulso, e o disparo único do relatório reprogramado virou uma routine do Claude Code em vez de repetir o mesmo padrão.
...
- `supabase/functions/daily-digest/index.ts` — public, gated by `x-cron-secret` header. Sends the day's itinerary + reminders (tasks due ≤ 48h, flights ≤ 24h, height alerts for the youngest minor) to every participant with a phone, at each tenant's `digest_time`/`timezone`. Originally scheduled by pg_cron (`'whatsapp-daily-digest'`, hourly) in `20260825130000_daily_digest_cron.sql`; moved to Vercel Cron (`api/cron/daily-digest.ts`) in `20260914190000_move_cron_to_vercel.sql` — see "Cron do bot: Vercel Cron, não mais pg_cron" below.
...
**Avisos por horário.** `supabase/functions/activity-reminders/index.ts` (público, `verify_jwt = false`, gated por `x-cron-secret`), agendado nos minutos `2,9,16,23,30,37,44,51,58` (passo de ~7 min) para não concorrer com o digest (minuto 7) — originalmente por pg_cron, hoje por Vercel Cron (`api/cron/activity-reminders.ts`, ver "Cron do bot" mais abaixo). Toda a lógica de janela vive em `_shared/reminderScheduler.ts`, que é **puro e testado** (28 testes) — a function só faz I/O.
