    ...
    <div className="space-y-6 pb-20">
          ...
          return (
            <div
              ...
              className={`glass-card p-5 rounded-2xl border transition space-y-3 ${
                isCritical ? 'border-danger-500/40 bg-danger-950/10' :
                isWarning ? 'border-warning-500/30 bg-warning-950/10' : 'border-info-500/30'
              }`}
            >
              <div className="flex items-start justify-between gap-3">
                <div className="flex items-center gap-3">
                  <div className={`w-9 h-9 rounded-xl flex items-center justify-center font-bold shrink-0 ${
                    isCritical ? 'bg-danger-500/20 text-danger-400' :
                    isWarning ? 'bg-warning-500/20 text-warning-400' : 'bg-info-500/20 text-info-400'
                  }`}>
                    {isCritical ? <ShieldAlert className="w-5 h-5" /> : <AlertTriangle className="w-5 h-5" />}
                  </div>
                  <div>
                    <span className={`px-2 py-0.5 rounded text-[10px] font-bold uppercase ${
                      isCritical ? 'bg-danger-500/20 text-danger-400' : 'bg-warning-500/20 text-warning-400'
                    }`}>
                ...
                <button
                  ...
                  className="px-3 py-1.5 rounded-xl bg-ink-800 hover:bg-success-600/20 text-ink-300 hover:text-success-400 border border-ink-700 text-xs font-semibold transition shrink-0"
                >
                  Marcar Resolvido
                </button>
              </div>

              <p className="text-xs text-ink-300">{finding.description}</p>

              <div className="p-3 rounded-xl bg-ink-950/80 border border-ink-800 text-xs text-ink-200">
                <span className="font-bold text-info-400 block mb-0.5">Recomendação IA / Sistema:</span>
      ...
      {resolved.length > 0 && (
        <div className="space-y-3 pt-4 border-t border-ink-800">
          <h3 className="text-sm font-bold text-ink-400">Apontamentos Resolvidos ({resolved.length})</h3>
          {resolved.map(finding => (
            <div key={finding.id} className="p-3 rounded-xl bg-ink-900/40 border border-ink-800 flex items-center justify-between text-xs text-ink-400 opacity-60">
              ...
              <button
                onClick={() => toggleResolveAudit(finding.id)}
                className="text-[11px] text-info-400 hover:underline"
              >
                Reabrir
              </button>
            </div>
