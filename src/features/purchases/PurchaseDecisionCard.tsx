...
import { activeQuote } from '../../services/purchase/priceQuotes';
...
export const PurchaseDecisionCard: React.FC<Props> = ({ item, decision, quotes, onEdit, onDelete, onResearch, onAiResearch, onMarkBought }) => {
  ...
  const temUs = Boolean(activeQuote(quotes, item.id, 'US'));
  const temBr = Boolean(activeQuote(quotes, item.id, 'BR'));

  return (
    <div className="glass-card p-5 rounded-2xl border border-ink-800 space-y-3">
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0">
          <h4 className="font-bold text-base text-ink-100 truncate">{item.product_name}</h4>
          <p className="text-xs text-ink-400">{item.brand || 'Marca'} • {item.store_name || 'Loja EUA'}</p>
        </div>
        <div className="flex items-center gap-1.5 shrink-0">
          <button onClick={() => onEdit(item)} className="p-1.5 rounded-lg bg-ink-800 text-ink-300 hover:text-ink-100 transition">
      ...
      {decision.verdict === 'DADOS_INSUFICIENTES' ? (
        <div className="space-y-2">
          <p className="text-[11px] text-ink-400">
            {temUs ? 'EUA ✓ registrado' : 'EUA — falta cotação'} · {temBr ? 'Brasil ✓ registrado' : 'Brasil — falta cotação'}
            {' · '}
            é preciso ter os dois mercados para comparar.
          </p>
      ...
      {decision.verdict !== 'DADOS_INSUFICIENTES' && (
        <>
          <button
            ...
            className="w-full flex items-center justify-center gap-1.5 py-1.5 text-[11px] font-semibold text-ink-400 hover:text-ink-100 transition"
          >
